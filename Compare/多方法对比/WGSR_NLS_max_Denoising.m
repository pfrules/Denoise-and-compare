function Result=WGSR_NLS_max_Denoising(I,nim,Sigma,Way)
addpath([Way '/'])
Opts=WGSR_NLS_Opts_Set (Sigma, I);
Opts.nim=nim;
[Denoising ,AllResult,AllTime] =WGSR_NLS_max( Opts);
...
function  [Denoising ,AllResult,AllTime]=WGSR_NLS_max( Opts)
b              =   Opts.win;
gamma          =   Opts.gamma;
...
%    SigmaCol = Opts.lamada * sqrt(abs(repmat(Opts.nSig^2, 1, size(X,2)) - mean((NY - X).^2))); %Estimated Local Noise Level    
 SigmaCol = Opts.lamada * sqrt(abs(repmat(Opts.nSig^2, 1, size(X,2)) - mean((NY - X).^2)));
if iter==1  
    ...
    %           NL_A          =     zeros (b*b, Opts.nblk);
        for  i  =  1 : K  
            ...
            if iter==1
               ...
               W2=gaussmf(W2,[mean(SigmaCol) max(SigmaCol(blk_arr(:, i))) ]);
                W2=W2/max(W2);
%                W2(W2<minW)=minW;
            end
%             W2=W2/min(W2);
