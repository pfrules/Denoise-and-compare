function Result=WNNM_DeNoising(I,nim,Sigma,Way)
addpath([Way '/'])
Par= ParSet(Sigma);   
[Denoising ,AllResult,AllTime] =WNNMing(nim, I, Par );
...
function [Denoising ,AllResult,AllTime]   =  WNNMing( N_Img, O_Img, Par )
...
AllResult{iter}=GetAllResult(E_Img,O_Img);
AllTime{iter}=toc; 
fprintf( 'Iteration %d :PSNR = %2.2f,SSIM = %2.2f\n', iter, AllResult{iter}.PSNR, AllResult{iter}.SSIM);
    cnt   =  cnt + 1;
  if  iter >1
