function Result=NCSR_Denoising(IMin0,IMin,Sigma,Way)
addpath([Way '/'])

par =    Parameters_setting( Sigma );
par.I            =    IMin0;
par.nim          =    IMin;
[Denoising ,AllResult,AllTime] =NCSR(IMin, IMin0, par );
...
function  [Denoising ,AllResult,AllTime]=NCSR(n_im, ori_im, par)
par.step      =   1;
[h1 w1]     =   size(ori_im);
...
for i  =  1 : 3  
    ...
    Denoising{cnt}    = d_im;
    AllResult{cnt}=GetAllResult(d_im,ori_im);
    AllTime{cnt}=toc; 
    fprintf( 'Iteration %d : nSig = %2.2f, PSNR = %2.2f,SSIM = %2.2f\n', cnt, par.nSig, AllResult{cnt}.PSNR, AllResult{cnt}.SSIM);
    if  cnt >1
