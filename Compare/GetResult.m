function GetResult(IMout,I0,outpar,write_name)
sigma=outpar.sigma;
method=outpar.method;
time=outpar.time;
write_dir=outpar.write_dir;
PSNR  =   csnr( IMout, I0, 0, 0 );
SSIM  =  cal_ssim( IMout, I0, 0, 0 );
write_file_dir=[write_dir method] ;
if ~isdir(write_file_dir)
    mkdir(write_file_dir)
end
outfile  = [write_file_dir '\' write_name(1:end-4) num2str(sigma) '.txt'];
fid = fopen(outfile, 'w','n','GB2312');%,
fprintf(fid, 'time:');
fprintf(fid, '%15.13f ', time);
fprintf(fid, '\nPSNR:');
fprintf(fid, '%15.13f ', PSNR);
fprintf(fid, '\nSSIM:');
fprintf(fid, '%15.13f ', SSIM);
fprintf(fid, '\n');
fclose(fid);
end
